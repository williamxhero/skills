...
def run_fault_matrix(*, seed: str = "sr-07-seed-1", keep_artifacts: bool = False) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="spec-runner-fault-") as temporary:
        ...
        cases.append({"id": "cleanup_only_takeover_has_no_worker", "entrypoint": "public_cli", "exit_code": code, "expected": "reverify_delivery_without_run", "actual": cleanup.get("action", {}).get("state"), "passed": code == 0 and code_status == 0 and cleanup.get("action", {}).get("state") == "reverify_delivery" and cleanup_status.get("runs") == []})
