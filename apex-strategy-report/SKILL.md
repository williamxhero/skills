...
本次用户的“无监督做下去”授权，适用于其已置于范围内的 MarketHub/QuantResearch 研究中的非破坏、可回滚、owner-scoped 代码/测试、正常 commit/push、带健康门和回滚的常规部署，以及来源真实、staging/验证通过的窄范围幂等数据 upsert。它不覆盖购买/升级订阅、新凭证/账号、绕过 paywall、许可不明的导出/留存/生产导入、全表替换/删除、不可逆迁移或扩大研究范围；这些必须 `awaiting_authorization`。这不是单纯 `create_thread` 授权的自动扩权。修复 task 早退、未通过 live 验收或主 task 复验失败时，均把 observation 证据发回同一 incident task，不得把“已请求授权”或单一路线失败描述为完成。
