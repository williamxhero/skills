...
**Autonomous express route:** when the user has supplied a broad requirement and explicitly wants delivery with no interviews or approval checkpoints, use **`/implement-needs`**. It partitions the requirement into scoped specs, runs `to-spec` → `to-tickets` → whole-spec implementation for each scope, merges each completed spec into the default branch before starting the next, then builds, packages, deploys, and smoke-tests the final revision. It applies repository defaults and stops only at an objective authority or external-state blocker.
