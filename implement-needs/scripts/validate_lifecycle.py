...
def evaluate(path: Path):
    ...
    active=set(); returned=set(); verified=set(); archived=set(); release=[]; finals=[]
    for i,e in enumerate(events):
        ...
        if kind=="dispatch_spec":
            ...
            else: returned.add(task)
        elif kind=="verify":
            if task not in returned: reasons.append(f"event_{i}_verify_without_handoff")
            else: verified.add(task)
        elif kind=="archive":
            if task not in verified: reasons.append(f"event_{i}_archive_without_verify")
            ...
            release.append(kind)
        elif kind=="controller_final": finals.append(e)
    ...
    if finals:
        if len(finals)!=1 or finals[0].get("state")!="terminal_success": reasons.append("terminal_count_or_state")
        if release != list(RELEASE): reasons.append("terminal_before_complete_release")
        if active: reasons.append("terminal_with_active_spec")
    return ({"decision":"allow","archived_specs":sorted(archived),"release_events":release,"terminal_count":len(finals)},0) if not reasons else ({"decision":"reject","reasons":reasons},1)
