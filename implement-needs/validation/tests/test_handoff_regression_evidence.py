...
class HandoffRegressionEvidenceTests(unittest.TestCase):
    ...
    def test_example_matches_run_scoped_schema(self):
        self.validator.validate(self.example)

    def test_wrong_run_marker_is_rejected(self):
    ...
    def test_ticket_level_artifacts_are_rejected(self):
        evidence = copy.deepcopy(self.example)
    ...
    def test_out_of_scope_ticket_is_rejected(self):
        evidence = copy.deepcopy(self.example)
        evidence["ticket_issues"][1]["number"] = 150
        evidence["ticket_issues"][1]["url"] = (
            "https://github.com/williamxhero/skills/issues/150"
        )
