...
[`model-policy.json`](model-policy.json) is the sole machine-readable allow-list for Implement Needs. Its rank order is deterministic: `gpt-5.6-sol` < `gpt-5.6-terra` < `gpt-5.6-luna`, and `medium` < `high` < `xhigh`. A fallback must use a different, allow-listed pair that is no weaker on either rank and remains owned by the same SPEC task.
