...
Run `python -m unittest discover -s implement-needs/tests -p 'test_*.py'` after changing controller behavior. The black-box scenarios in `test_behavioral_acceptance.py` copy an installed-like skill artifact without its source test tree, invoke its validators under isolated Python mode, and drive observable messages, waits, task operations, recovery, route rejection, handoff verification, archival, release ownership, and terminal output through an isolated fixture; they do not assert prose wording.
