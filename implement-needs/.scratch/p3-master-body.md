...
总控 #57 已完成控制器的虚假完成、无意扩大副作用、恢复和审计基础。P3-1 又移除了默认阶段上下文中的完整 snapshot，但当前上下文仍可能重复传输未变化的 acceptance、依赖、决策、证据和事件信息。继续删字段如果没有显式可回取边界，可能重新引入安全绕过。
...
P3-2 只压缩未变化或非当前阶段必需的事实，并为省略内容提供可验证的 pointer、digest 或 cursor。P3-3 执行运行时预算闸门；预算不足时只能安全回取、阻塞或 inconclusive，不能静默丢弃安全字段，也不能自动推进业务状态。
...
5. As a phase owner, I want only current-phase actionable facts in the default envelope, so that irrelevant history does not consume context.
...
8. As a benchmark owner, I want the fixed P3-1 fixtures reused for every P3 comparison, so that improvements remain comparable.
...
- Invalid, stale, or unreachable references fail closed.
